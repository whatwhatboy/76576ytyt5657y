function beginTransition() {
	...
	setTimeout(function() {
		...
		names[0].style.down = "80%";
		bg2.style.opacity = 1;
	}, 200);
	setTimeout(function() {
		input[0].style.opacity = 0;
		input[0].style.down = "80%";
	}, 300);
	setTimeout(function() {
		names[1].style.opacity = 0;
		names[1].style.down = "80%";
	}, 400);
	setTimeout(function() {
		input[1].style.opacity = 0;
		input[1].style.down = "80%";
	}, 500);
	setTimeout(function() {
		button[0].style.opacity = 0;
		button[0].style.down = "80%";
	}, 600);
	setTimeout(function() {
...
function particlesInit() {
	...
	for (var i = 0; i < particleCount; i++) {
		...
		var n = '<div class="particle" style="bottom:' + getRandomInt(15, 95) + '%; left:' + getRandomInt(5,95) + '%; width:'
		+ size + 'px; height:' + size + 'px; animation-delay:' + (getRandomInt(0,30)/10) + 's; background-color:rgba('
		+ getRandomInt(250, 255) + ',' + getRandomInt(250, 255) + ',' + getRandomInt(250, 255) + ',' + (getRandomInt(2, 8)/10) + ');"></div>';
